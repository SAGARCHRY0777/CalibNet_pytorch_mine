...
def calculate_ground_truth_pcd(original_pcd, igt_inverse):
    ...
    # original_pcd is expected to be of shape (B, 3, N)
    B, _, N = original_pcd.shape
    
    # Create a row of ones with shape (B, 1, N)
    ones_row = torch.ones((B, 1, N), dtype=original_pcd.dtype, device=original_pcd.device)
    
    # Concatenate original_pcd (B, 3, N) with ones_row (B, 1, N) along dimension 1
    # This results in pcd_hom of shape (B, 4, N)
    pcd_hom = torch.cat((original_pcd, ones_row), dim=1)
    
    # Apply inverse transformation.
    # igt_inverse is expected to be (B, 4, 4)
    # pcd_hom is (B, 4, N)
    # The result gt_pcd_hom will be (B, 4, N)
    gt_pcd_hom = igt_inverse @ pcd_hom
    
    # Convert back from homogeneous coordinates to 3D by dividing x, y, z by the homogeneous coordinate (w)
    # Slicing gt_pcd_hom[:, :3, :] takes the first 3 rows (x, y, z) for all batches and points.
    # Slicing gt_pcd_hom[:, 3:4, :] takes the 4th row (w) for all batches and points, keeping dimension for broadcasting.
...
def test(args, chkpt: dict, test_loader):
    ...
    model.to(device)
    # Ensure strict=False if the checkpoint does not exactly match the model architecture
    # based on previous errors. For clean execution, a matching checkpoint is ideal.
